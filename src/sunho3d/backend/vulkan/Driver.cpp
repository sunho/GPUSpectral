...
void VulkanDriver::drawFrame() {
    uint32_t imageIndex;
    vkAcquireNextImageKHR(context.device, surface.swapChain, UINT64_MAX, surface.imageAvailableSemaphore, VK_NULL_HANDLE, &imageIndex);
    ...
    submitInfo.pCommandBuffers = &surface.swapContexts[imageIndex].commands;
...
void VulkanDriver::fillCommands() {
    framebuffers.resize(surface.size);
    for (int i = 0; i < surface.size; ++i) {
        VulkanSwapContext& ctx = surface.swapContexts[i];
        ...
        if (vkBeginCommandBuffer(ctx.commands, &beginInfo) != VK_SUCCESS) {
        ...
        vkCmdBeginRenderPass(ctx.commands, &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);
        vkCmdBindPipeline(ctx.commands, VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipeline);
        vkCmdDraw(ctx.commands, 3, 1, 0, 0);
        vkCmdEndRenderPass(ctx.commands);
        if (vkEndCommandBuffer(ctx.commands) != VK_SUCCESS) {
...
void VulkanDriver::createPipeline() {
    VkShaderModule vertShaderModule = createShaderModule(triangle_vert, triangle_vert_len);
    VkShaderModule fragShaderModule = createShaderModule(triangle_frag, triangle_frag_len);
...
VkShaderModule VulkanDriver::createShaderModule(const char* code, size_t codeSize) {
