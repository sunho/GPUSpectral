...
RenderTargetHandle VulkanDriver::createDefaultRenderTarget(int dummy) {
    ...
    auto tex = VulkanTexture(context, SamplerType::SAMPLER2D, TextureUsage::DEPTH_ATTACHMENT, 1,
...
void VulkanDriver::draw(PipelineState pipeline, PrimitiveHandle handle) {
    ...
    for (uint32_t i = 0; i < bufferCount; ++i) {
        ...
        attributes[i] = { .location = i,
                          .binding = i,
                          .format = getVkFormat(attrib.type,
    ...
    VulkanPipelineKey key = {
        .attributes = attributes, .bindings = bindings, .program = program, .viewport = vp
    };
