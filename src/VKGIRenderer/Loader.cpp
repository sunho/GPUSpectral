...
static void loadMesh(Renderer& renderer, Mesh* mesh, const std::string& path) {
    ...
    for (size_t s = 0; s < shapes.size(); s++) {
        ...
        auto buffer0 = driver.createBufferObject(4 * v.size(), BufferUsage::VERTEX | BufferUsage::STORAGE, BufferType::DEVICE);
        driver.updateBufferObjectSync(buffer0, { .data = (uint32_t*)v.data() }, 0);
        mesh->positionBuffer = buffer0;
        auto buffer1 = driver.createBufferObject(4 * v.size(), BufferUsage::VERTEX | BufferUsage::STORAGE, BufferType::DEVICE);
        driver.updateBufferObjectSync(buffer1, { .data = (uint32_t*)vn.data() }, 0);
        mesh->normalBuffer = buffer1;
        auto buffer2 = driver.createBufferObject(4 * vt.size(), BufferUsage::VERTEX | BufferUsage::STORAGE, BufferType::DEVICE);
